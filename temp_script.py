...
# 1. Remove from dashboard.py
with open(r'c:\Users\DELL\Desktop\Financial_Profiler_FYP\src\dashboard.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

new_dash_lines = []
for line in lines:
    if '# --- ACTIVE LEGAL VIEW RENDERER ---' in line:
        break
    new_dash_lines.append(line)

with open(r'c:\Users\DELL\Desktop\Financial_Profiler_FYP\src\dashboard.py', 'w', encoding='utf-8') as f:
    f.writelines(new_dash_lines)

# 2. Add to utils.py
content = """    # --- ACTIVE LEGAL VIEW RENDERER ---
    active_view = st.session_state.get('active_legal_view')
    if active_view:
        st.markdown("<hr style='border-color: rgba(255,255,255,0.1); margin: 40px 0;'>", unsafe_allow_html=True)
        col_close1, col_close2 = st.columns([9, 1])
        with col_close2:
            if st.button("✖ Close", key="close_legal", use_container_width=True):
                st.session_state['active_legal_view'] = None
                st.rerun()
        
        if active_view == 'Privacy Policy':
            st.markdown('''
        ...
        elif active_view == 'Terms of Service':
            st.markdown('''
            <div style="text-align: center; margin-bottom: 50px;">
        ...
        elif active_view == 'Cookie Policy':
            st.markdown('''
            <div style="text-align: center; margin-bottom: 50px;">
